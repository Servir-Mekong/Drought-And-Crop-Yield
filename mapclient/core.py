...
class GEEApi():
    ...
    def __init__(self):
        ...
        self.MKBoundary = ee.FeatureCollection(settings.MKBOUNDARY)
        self.MKADM0 = ee.FeatureCollection(settings.MKADM0)
        self.MKADM1 = ee.FeatureCollection(settings.MKADM1)
        self.MekongRiverBasin = ee.FeatureCollection(settings.MEKONGRIVERBASIN)
        self.geometry = self.MKADM0.filter(ee.Filter.eq('NAME_0', 'Cambodia')).geometry()
    ...
    def get_map_id(self, date, dataset):
        ...
        image = image.updateMask(self.maskedArea).clip(self.geometry)
