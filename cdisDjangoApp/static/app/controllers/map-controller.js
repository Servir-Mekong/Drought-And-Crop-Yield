(function () {
  ...
  .controller('MapController', function ($http, $rootScope, $scope, $sanitize, $timeout, appSettings, MapService) {
    ...
    /**
		...
		map.getPane('admin').style.zIndex = 565;
		...
		L.Control.Custom = L.Control.Layers.extend({
			...
			_addElement: function () {
				...
				'<ul class="toggles-list">'+
					'<li class="toggle"><label class="switch_layer"><input  name="inland_water_toggle" id="inland_water_toggle" checked="checked" type="checkbox"><span class="slider_toggle round"></span></label><label>Permanent Water </label></li>'+
				'</ul>';
			}
		});
    ...
    // Load water area Geojson
		 $.getJSON('/static/data/perennial.geo.json')
    ...
    function onEachCountry(feature, layer) {
      ...
      layer.on("click", function (e) {
          ...
          if(type=="admin1") {
            ...
            $("#selectedAreaText").text("Cambodia | " + selectedArea );
          }else if(type=="admin2"){
  ...
  // function to add and update tile layer to map
    ...
    $scope.downloadRaster = function(){
      ...
      if(sensortype === 'sb'){
        ...
        selected_date = selected_date.replace('-', '_');
        DownloadURL = $scope.downloadServerURL + '/rdcyis_outputs/eo_based/'+ selectedopt + '/cambodia/' +selectedopt+'_'+selected_date+'_cambodia.tif';
      }else{
...
function genAreaChart(categoriesData, data1, average, chartid){
  ...
  $('#toggle_disclaimer').click(function(){
    $('#toggle_drought_box').css("display", "none");
    $('#toggle_drought').removeClass("active");
    if($('#toggle_disclaimer_box').css("display") === "none"){
      $('#toggle_disclaimer_box').css("display", "block");
      $(this).addClass("active");
    }else{
      $('#toggle_disclaimer_box').css("display", "none");
      $(this).removeClass("active");
